...
{
    public class TenantRedisSetCacheService : ITenantSetCahceService
    {
        private readonly CSRedisClient _redisClient;
        private ICXLUserClaimsService _cXLUserClaimsService;

        public TenantRedisSetCacheService(CSRedisClient redisClient, ICXLUserClaimsService cXLUserClaimsService)
        {
        ...
        public async Task<HashSet<string>> IntersectSetsAsync(params string[] keys)
        {
            if (keys == null || keys.Length == 0)
                throw new ArgumentException("密钥不能为 null 或空。");
            keys = keys.Select(key => key = this.GetTenantKey(key)).ToArray();
        ...
        public async Task<HashSet<string>> DifferenceSetsAsync(string key1, string key2)
        {
            if (string.IsNullOrWhiteSpace(key1) || string.IsNullOrWhiteSpace(key2))
                throw new ArgumentException("密钥不能为 null 或空。");
            key1 = this.GetTenantKey(key1);
            key2 = this.GetTenantKey(key2);
